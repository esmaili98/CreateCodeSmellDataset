...
class clsComputeMetricsForInputProject:
    ...
    def computeMetrics(self):
        ...
        MethodMetrics=obj.returnMethodMetrics(db)
        udbadd=str(db)
        return [MethodMetrics,udbadd]

    def saveCSVFile(self):
        computemetrics_ouptput=self.computeMetrics()
        methodMetrics=computemetrics_ouptput[0]
        ...
        # print(methodMetrics)
        methodMetrics[1].insert(0, 'longname')
        for i in range(len(methodMetrics[0])):
            methodMetrics[2][i].insert(0, methodMetrics[0][i])

        for i in range(len(methodMetrics[2])):
            methodMetrics.append(methodMetrics[2].pop(0))
        # print(methodMetrics)
        methodMetrics.pop(0)
        methodMetrics.pop(1)
        # print(methodMetrics)
        desktopadd = os.path.join(os.path.join(os.environ['USERPROFILE']), 'Desktop')
        address= desktopadd+'/Prediction/'+ udbName +'/MethodSmells/' + udbName + 'MethodMetrics.csv'
        if  not os.path.exists(desktopadd +'/Prediction/'+ udbName +'/MethodSmells/'):
            os.makedirs(desktopadd+'/Prediction/'+ udbName +'/MethodSmells/')

        with open(address,'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerows(methodMetrics)
...
class clsCleanProjectMetrics:
    # address='C:/Users/Sadaf/Desktop/Prediction/MethodMetrics/' + 'projectName' + '.csv'
